...
type
  FrameRequest {.bycopy.} = object
    ...
    frames: Table[int,ptr VSFrame]#ptr VSFrameRef]
...
proc y4mheader(node:VSNodeObj):string =
  ...
  if not (vinfo.handle.format.colorFamily in [cfYUV.cint, cfGray.cint]):
    raise newException(ValueError, ".y4m only supports YUV and Gray formats")
  var format = ""

  let bitsPerSample = vinfo.handle.format.bitsPerSample
  if vinfo.handle.format.colorFamily == cfGray.cint:
    if bitsPerSample > 8: 
      format &= &"mono{bitsPerSample}"
    else:
      format &= "mono"

  elif vinfo.handle.format.colorFamily == cfYUV.cint:
    # https://en.wikipedia.org/wiki/Chroma_subsampling#Types_of_sampling_and_subsampling
    # subSamplingW: 2nd and 3rd plane sampling in horizontal direction
    # subSamplingH: same for vertical direction
    let ssW = vinfo.handle.format.subSamplingW.int
    let ssH = vinfo.handle.format.subSamplingH.int
...
proc frameDoneCallback( reqsData: pointer; 
  ...
  if frame != nil:
    reqs.frames[n.int] = frame  # Store the new frame in the buffer (global)
    #echo "Completed: ", n
    ...
    while reqs.frames.hasKey( k ):
      ...
      let format = getVideoFrameFormat(frame) #f.getFrameFormat#.toFormat    
      ...
      for i in 0..<format.numPlanes:
        ...
        let init = frame.getReadPtr( i)
        let stride = frame.getStride(i)
        for row in 0..<height:
    ...
    if reqs.requestedFrames < reqs.numFrames:
      api.handle.getFrameAsync( reqs.requestedFrames.cint, node, frameDoneCallback, nil)
...
proc writeY4mFramesAsync(strm:FileStream, node:VSNodeObj):int =
  ...
  reqs.frames = initTable[int,ptr VSFrame]()  # Buffer
  initLock(lock)
  for i in 0..<initialRequest: 
    api.handle.getFrameAsync( i.cint, node.handle, frameDoneCallback, nil) #dataInHeap)
    #echo "Requested: ", i
    reqs.requestedFrames += 1
  cond.wait(lock)
  #API.freeMap(vsmap)
...
proc doNothing( reqsData: pointer, 
  ...
  # Do something with the frame
  api.handle.freeFrame( frame )
  reqs.completedFrames += 1

  # Once a frame is completed, we request another frame while there are available
  if reqs.requestedFrames < reqs.numFrames:
    api.handle.getFrameAsync( reqs.requestedFrames.cint, node, doNothing, reqsData)
...
proc Null*(vsmap:VSMapObj):int =
  ...
  initLock(lock)
  for i in 0..<initialRequest:  #
    api.handle.getFrameAsync( i.cint, node.handle, doNothing, nil) #dataInHeap)
    #echo "Frame: ", i
    reqs.requestedFrames += 1

  cond.wait(lock)
  #api.handle.freeMap(vsmap)
  #API.freeNode(node)
  return reqs.numFrames
